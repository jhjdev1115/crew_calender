...
export async function GET(request: Request) {
  try {
    ...
    const connection = await context.db
      .prepare(
        `SELECT c.*, p.user_id AS partner_id, p.display_name, p.role, p.airline, p.base_airport
    ...
    return apiError(error, "연동 정보를 불러오지 못했어요.");
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "accept") {
      ...
      const invite = await context.db
        .prepare(
          `SELECT i.id, i.issuer_user_id, p.role FROM invite_codes i JOIN profiles p ON p.user_id = i.issuer_user_id
        WHERE i.code_hash = ? AND i.used_at IS NULL AND i.revoked_at IS NULL AND i.expires_at > ?`,
        )
        .bind(hash, now)
        .first<{ id: string; issuer_user_id: string; role: string }>();
      if (!invite || invite.issuer_user_id === context.user.userId)
        return Response.json(
          { error: "사용할 수 없는 코드예요." },
          { status: 422 },
        );
      const accepter = await context.db
        .prepare("SELECT role FROM profiles WHERE user_id = ?")
        .bind(context.user.userId)
        .first<{ role: string | null }>();
      if (
        !accepter?.role ||
        (accepter.role === "partner" && invite.role === "partner")
      )
        return Response.json(
          { error: "이 역할 조합은 연동할 수 없어요." },
          { status: 422 },
        );
      const existing = await context.db
        .prepare(
          "SELECT user_id FROM active_memberships WHERE user_id IN (?, ?) LIMIT 1",
        )
        .bind(context.user.userId, invite.issuer_user_id)
        .first();
      if (existing)
        return Response.json(
          { error: "이미 활성 연동이 있어요." },
      ...
      await context.db.batch([
        context.db
          .prepare(
            "INSERT INTO connections (id, user_low_id, user_high_id, status, linked_at) VALUES (?, ?, ?, 'active', ?)",
          )
          .bind(connectionId, low, high, now),
        context.db
          .prepare(
            "INSERT INTO active_memberships (user_id, connection_id, created_at) VALUES (?, ?, ?)",
          )
          .bind(context.user.userId, connectionId, now),
        context.db
          .prepare(
            "INSERT INTO active_memberships (user_id, connection_id, created_at) VALUES (?, ?, ?)",
          )
          .bind(invite.issuer_user_id, connectionId, now),
        context.db
          .prepare(
            "UPDATE invite_codes SET used_at = ? WHERE id = ? AND used_at IS NULL",
          )
          .bind(now, invite.id),
      ]);
    ...
    if (body.action === "unlink") {
      ...
      const membership = await context.db
        .prepare(
          "SELECT connection_id FROM active_memberships WHERE user_id = ?",
        )
        .bind(context.user.userId)
      ...
      if (membership)
        await context.db.batch([
          context.db
            .prepare(
              "UPDATE connections SET status = 'unlinked', unlinked_at = ? WHERE id = ? AND status = 'active'",
            )
            .bind(now, membership.connection_id),
          context.db
    ...
    return apiError(error, "연동 요청을 처리하지 못했어요.");
  }
}
