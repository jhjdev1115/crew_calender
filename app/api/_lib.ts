...
export async function prepareRequest(request: Request): Promise<{ user: ApiUser; db: D1Database } | Response> {
  const user = getRequestUser(request);
  if (!user) return Response.json({ error: "로그인이 필요합니다." }, { status: 401 });
  await ensureDatabase();
  const db = getD1();
  const now = new Date().toISOString();
  await db.prepare(`INSERT INTO profiles (user_id, email, display_name, schedule_tz, created_at, updated_at)
    VALUES (?, ?, ?, 'Asia/Seoul', ?, ?)
  ...
  await db.prepare(`INSERT OR IGNORE INTO notification_preferences (user_id, updated_at) VALUES (?, ?)`)
...
export function getRequestUser(request: Request): ApiUser | null {
  ...
  if (userId && email) {
    ...
    if (encoded && request.headers.get("oai-authenticated-user-full-name-encoding") === "percent-encoded-utf-8") {
      try { displayName = decodeURIComponent(encoded); } catch { /* fall back to email */ }
    }
    return { userId, email, displayName };
  }
  const hostname = new URL(request.url).hostname;
  if (hostname === "localhost" || hostname === "127.0.0.1") {
    return { userId: "local-development-user", email: "local@crewsync.dev", displayName: "지원" };
  }
  return null;
}
...
export function toBool(value: unknown): boolean { return value === true || value === 1; }

export function monthBounds(value: string | null): { start: string; end: string } | null {
...
export async function hashInviteCode(code: string, request: Request): Promise<string> {
  const local = ["localhost", "127.0.0.1"].includes(new URL(request.url).hostname);
  const pepper = getInvitePepper() ?? (local ? "crewsync-local-development-pepper" : null);
...
export function apiError(error: unknown, fallback = "요청을 처리하지 못했어요.") {
